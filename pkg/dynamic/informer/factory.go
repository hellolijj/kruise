...
func (f *SharedInformerFactory) Resource(apiVersion, resource string) (*ResourceInformer, error) {
	...
	if sharedInformer, ok := f.sharedInformers[key]; ok {
		...
		glog.Infof("Subscribed to shared informer for %v in %v (total subscribers now %v)", resource, apiVersion, count)
	...
	closeFn := func() {
		...
		count := f.refCount[key] - 1
		glog.Infof("Unsubscribed from shared informer for %v in %v (total subscribers now %v)", resource, apiVersion, count)
		...
		glog.Infof("Stopping shared informer for %v in %v (no more subscribers)", resource, apiVersion)
		close(stopCh)
		delete(f.refCount, key)
		delete(f.sharedInformers, key)
	}

	glog.Infof("Starting shared informer for %v in %v", resource, apiVersion)
